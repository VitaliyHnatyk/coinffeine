...
/** Wrapper for nubitsj transactions.
  *
  * As nubitsj transactions are mutable we need a source of fresh objects to keep our sanity.
...
class ImmutableTransaction(private val bytes: Array[Byte], private val network: Network) {
  ...
  def this(tx: MutableTransaction) = this(tx.nubitsSerialize(), tx.getParams)
